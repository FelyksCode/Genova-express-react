...
router.post("/register/:race_id", upload.fields(fields), (req, res) => {
  ...
  try {
    ...
    if (!req.files || req.files.length === 0) {
      return res
        .status(400)
        .json({ error: true, message: "At least one picture is required." });
    }
    if (req.fileValidationError) {
      return res
        .status(400)
        .json({ error: true, message: req.fileValidationError });
    }
    ...
    for (const userData of users) {
      ...
      const validate = userSchema.validatePayload(
        payload,
    ...
    return res
      .status(500)
      .json({ error: true, message: "Internal server error: " + e.message });
  }
});
