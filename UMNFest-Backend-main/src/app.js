...
app.get('/', (req, res) => {
    res.json({ error: false, "message": "UMNFest API", "docs": "https://elements.getpostman.com/redirect?entityId=33191415-b1a4909c-0b6d-4c0b-a65c-ea0e640f9935&entityType=collection" });
});
...
function firstStart() {
    ...
    RaceDetail.estimatedDocumentCount()
        .then(count => {
            const raceNames = ["Unveiling", "E-Ulympic", "Ulympic", "U-Care", "Unify"];
            const raceDescription = ["Lomba Unveiling adalah...", "Lomba E-ulympic adalah...", "Lomba Ulympic adalah...", "Lomba U-Care adalah...", "Lomba Unify adalah"];
            if (count === 0 || count != raceNames.length) {
                var i = 0;
                raceNames.forEach(raceName => {
                    new RaceDetail({ race_name: raceName, race_description: raceDescription[i] }).save()
                        .then(() => {
